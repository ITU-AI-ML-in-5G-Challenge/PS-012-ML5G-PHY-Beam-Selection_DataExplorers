...
class ModelHandler:
    ...
    def createArchitecture(self,model_type,num_classes,input_shape,chain):
        ...
        if(model_type == 'inception_single'):
            ...
            tower_1 = Conv2D(64, (5,5), padding='same', activation='relu')(input_inc)
            tower_1 = MaxPooling2D((2,2), strides=(1,1), padding='same')(tower_1)
            tower_1 = Dropout(0.2)(tower_1)
            tower_1 = Flatten()(tower_1)
            output = Dense(64,activation='relu')(tower_1)
            out = Dense(num_classes,activation='softmax')(output)
            architecture = Model(inputs = input_inc, outputs = out)
            ...
            layer = Dense(32,activation='relu')(input_coord) 
            ...
            layer = Conv2D(8,kernel_size=(5,5),
                                activation='relu',
                                padding="SAME",
                                input_shape=input_shape,kernel_regularizer=regularizers.l2(0.008))(input_lid)
            layer = MaxPooling2D((2,2), strides=(1,1), padding='same')(layer)
            layer = Dropout(0.2)(layer)
            layer = Conv2D(16, (5,5), padding='same', activation='relu',kernel_regularizer=regularizers.l2(0.008))(layer)
            layer = MaxPooling2D((2,2), strides=(1,1), padding='same')(layer)
            layer = Dropout(0.2)(layer)
            layer = Flatten()(layer)
            layer = Dense(512,activation='relu',kernel_regularizer=regularizers.l2(0.008))(layer)
            layer = Dropout(0.2)(layer)
            out = layer
